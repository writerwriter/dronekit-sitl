...
def TaskDone(db,multi_waypoint_mission,is_abandoned):
	...
	for waypoint_mission in multi_waypoint_mission:
		...
		sql = "INSERT iNTO mission_results(waypoint_id, mission_latitude, mission_longitude, mission_id, is_abandoned) VALUES (%d,%.15f,%.15f,%d,%i)" % (waypoint_mission.waypoint_id, waypoint_mission.mission_latitude, waypoint_mission.mission_longitude, waypoint_mission.mission_id, is_abandoned)
		cursor = getCursor(db)
		try:
			cursor.execute(sql)
			cursor.close()
